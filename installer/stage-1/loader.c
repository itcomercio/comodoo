...
static void spawnShell(void) {
    ...
    logMessage(INFO, "spawn shell setup");
    ...
    if (access("/bin/sh",  X_OK)) 
        logMessage(ERROR, "cannot open shell - /bin/sh doesn't exist, trying /bin/bash");

    if (access("/bin/bash",  X_OK))  {
        logMessage(ERROR, "cannot open shell - /bin/bash doesn't exist");
	return;
    } else 
    	logMessage(INFO, "/bin/bash open with success");
	...
	int fd;
    	...
    	logMessage(INFO, "/dev/tty2 ready for shell");
	...
	isysLoadFont();
    	...
    	logMessage(INFO, "setsid to background, new controlling tty");
        ...
        if (execl("/bin/bash", "/bin/bash", "-i", NULL) == -1) {
            logMessage(CRITICAL, "exec of /bin/sh or /bin/bash failed: %m");
            exit(1);
        }
    }
...
static void parseCmdLineFlags(struct loaderData_s * loaderData,
    ...
    for (i=0; i < argc; i++) {
        ...
        else if (!strcasecmp(argv[i], "inter"))
            flags |= LOADER_FLAGS_INTERACTIVE;
...
static char *doLoaderMain(struct loaderData_s *loaderData,
    ...
    if (!FL_ASKMETHOD(flags) && !loaderData->stage2Data) {
        ...
        /* en este punto ya esta en RAM el installer stage-2 anaconda */
        if (url) {
...
static void migrate_runtime_directory(char *dirname) {
    ...
    if (!access(runtimedir, F_OK)) {
        if (unlink(dirname) == -1) {
    	    ...
    	    logMessage(INFO, "%s: rename %s to %s", __func__, dirname, olddir);
            ret = rename(dirname, olddir);
            free(olddir);
        }
    	logMessage(INFO, "%s: link %s to %s", __func__, runtimedir, dirname);
        ret = symlink(runtimedir, dirname);
	if (ret)
		logMessage(CRITICAL, "%s: %d: %m link error for %s", 
...
int main(int argc, char **argv) {
    ...
    if (connection == NULL) {
        logMessage(INFO, "connection == NULL! ...");
        ...
        logMessage(INFO, "iface_start_NetworkManager ...");
    ...
    while (path && path[0]) {
        ...
        logMessage(INFO, "binpath: %s", binpath);
